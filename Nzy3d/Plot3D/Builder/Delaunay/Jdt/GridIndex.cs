namespace Nzy3d.Plot3D.Builder.Delaunay.Jdt
{
	...
	public class GridIndex
	{
		...
		public GridIndex(Delaunay_Triangulation delaunay, int xCellCount, int yCellCount)
			: this(delaunay, xCellCount, yCellCount, delaunay.BoundingBox)
		{
		}
		...
		private Triangle_dt FindValidTriangle(Point_dt minInvalidCell)
		{
			...
			else
			{
				...
				return grid[(int)MathF.Min(0, minInvalidCell.X), (int)MathF.Min(0, minInvalidCell.Y)];
			}
		}
		...
		{
			float middleXCell = indexRegion.MinX + x_index * x_size + x_size / 2f;
			float middleYCell = indexRegion.MinY + y_index * y_size + y_size / 2f;
